...
const jobProcessor = async (job: any) => {
    ...
    if (!endpoint || !endpoint.user || !endpoint.isActive) {
        ...
        const repeatableJobs = await cleanupQueue.getJobSchedulers();
        const jobToRemove = repeatableJobs.find((j) => j.id === `endpoint-${endpointId}`);
        if (jobToRemove) {
            await cleanupQueue.removeJobScheduler(jobToRemove.key);
            console.log(`[Ping Worker] Cleaned up obsolete job for endpoint ${endpointId}`);
        }
        return;
    }
