...
export const createEndPoint = async (req, res) => {
    try {
        ...
        const parsedInterval = parseIntParam(checkIntervalSec?.toString(), 60, 30, 3600);
        const parsedAlertThreshold = parseIntParam(alertOnConsecutiveFails?.toString(), 4, 1, 20);
        ...
        await pingQueue.add('ping-job', { endpointId: newEndpoint.id, url: newEndpoint.url }, {
...
export const pauseEndPoint = async (req, res) => {
    try {
...
export const resumeEndpoint = async (req, res) => {
    try {
        const endpointId = parseIntParam(req.params.id, 0);
        if (endpointId === 0) {
            return res.status(400).json({ error: 'Invalid endpoint ID' });
        ...
        try {
            await pingQueue.add('ping-job', { endpointId: updatedEndpoint.id, url: updatedEndpoint.url }, {
...
export const getEndpointMetrics = async (req, res) => {
    try {
        ...
        const startTime = timeRanges[range] || timeRanges['1h'];
