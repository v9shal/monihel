...
export const getHourlyMetrics = async (req, res) => {
    try {
        ...
        const rangeHours = parseIntParam(req.query.range, 24, 1, 168);
...
export const getDailyMetrics = async (req, res) => {
    try {
        ...
        const rangeDays = parseIntParam(req.query.range, 30, 1, 365);
        const result = await prisma.$queryRaw `
            SELECT 
                day, 
                avg_response_time, 
